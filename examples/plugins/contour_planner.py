...
主程序只提供"单向/往复"两种最基础的栅格刀路，其余留给大家练手。这个文件就是一个完整的
例子：它自己带着所需的几何（多边形等距偏置），只依赖公开的接口。
...
3. 重启程序——界面"刀路"分组里就会出现"环切(等距轮廓)"，参数控件自动生成。

**可以继续练手的方向**：凹形状的窄颈处会出现"环断裂"，现在只保留一条环；
把一圈拆成多条环、或者做成螺旋插补，都是很好的下一步。
"""
